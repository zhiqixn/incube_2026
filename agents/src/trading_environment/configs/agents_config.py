...
user_cfgs = [
    {
        ...
        "agent_topic_type": "fundamentals_analyst",
    }
]
...
they are defined parameters,
otherwise your call will fail. Make sure to call get_YFin_data first to retrieve
the CSV that is needed to generate
indicators. Call get_stock_stats_indicators_tool to retrieve chosen indicators.
...
in the stock. You must also address reflections and learn from lessons and mistakes
...
bull position. You must also address reflections and learn from lessons and mistakes
you made in the past.
""",
        "model": model,
        "delegate_tools": [delegate_tasks],
...
recommendation. Do not forget to utilize lessons from past decisions to learn from
your mistakes. Here is some reflections from similar situatiosn you traded in and
the lessons learned: {past_memory_str}
Based on a comprehensive analysis by a team of analysts, here is an investment
...
Investment Plan: {investment_plan}\n\nLeverage these insights to make an informed
...
from the conservative analyst: {current_safe_response}. If there are no responses
...
cautious alternatives could secure long-term gains. Here is the trader's decision:

{trader_decision}

Your task is to actively counter the arguments of the Risky Analyst, highlighting
...
the risky analyst: {current_risky_response}. If there are no responses from the other
...
address prior misjudgments and improve the decision you are making now to make
